...
-- The worker now returns immediately, but we also increase timeout as backup

-- Update trigger_daily_digest to use longer timeout (30 seconds)
...
DECLARE
  request_id bigint;
BEGIN
  -- Call the Cloudflare Worker endpoint using pg_net with 30s timeout
  -- Worker returns immediately and runs work in background
...
BEGIN
  ...
  IF digest_count = 0 THEN
    -- No digest today - trigger generation with 30s timeout
    SELECT net.http_post(
      url := 'https://podgest-api.pztest.workers.dev/api/daily-cron'::text,
      headers := '{"Content-Type": "application/json"}'::jsonb,
      body := '{}'::jsonb,
