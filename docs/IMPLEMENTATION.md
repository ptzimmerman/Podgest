# Podgest - Implementation Plan & Design Docs

> This document contains the original design plans, architecture decisions, implementation roadmap, and technical notes for the Podgest platform. For a high-level overview, see the main [README](../README.md).
...
└─────────────────┘     └─────────────────┘     └────────┬────────┘
         ...
         ▼
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│  pgvector    │────▶│  Claude: Generate│────▶│  Modal: TTS     │
...
| **Semantic chunks** | Supabase pgvector | Embeddings, search, RAG |
| **Generated audio** | Supabase Storage (public) | CDN delivery for podcast RSS |

> **Note:** pgvector is native to Supabase PostgreSQL. Filtering by `user_id`, `date_range`, and `podcast_name` is handled via standard SQL WHERE clauses alongside vector similarity search. No external service needed.
...
const chunks = chunkText(transcript.text, 500, 50);
const embeddings = await openai.embeddings.create({
  model: 'text-embedding-3-small',
  input: chunks,
});

// 4. Insert chunks with embeddings into pgvector table
...
| **Memory Backend** | pgvector API | Semantic search across transcripts |
...
|----------|---------|------|---------|
| `search_podcasts` | pgvector chunks (excerpts) | ~500 words each | <1s |
...
| Embeddings + search | pgvector (cloud) |
...
| **Supabase pgvector** | Native embeddings + vector search, no external service needed |
...
| **Full documents in Postgres** | Bloats DB, slows queries | Tiered storage (Postgres → S3 → pgvector) |
...
│  │         │                                                                    ││
│  │         ▼                                                                    ││
│  │   Extract topics (Claude) ──▶ Embed in pgvector                          ││
│  │                        │                                                     ││
│  │                        ▼                                                     ││
...
| Semantic chunks | pgvector | Search, embeddings, RAG |
...
export async function handleModalWebhook(request, env) {
  ...
  // Embed in pgvector
  await embedInPgvector(env, job_id);
...
CREATE TABLE public.transcriptions (
  ...
  embedding_id TEXT,          -- Reference to pgvector embedding
...
| pgvector | Included in Supabase | ~$0 | Native to PostgreSQL |
...
| **Search** | pgvector for semantic search + embeddings |
...
- [x] **Verify pgvector** supports user_id, date, podcast filtering ✅ (user_id column + SQL WHERE clauses)
...
- [x] `embed-content` function (stores in pgvector) — auto-triggers after topic extraction
...
- [x] pgvector query integration in MCP tools (`search_podcasts`, `compare_takes`)
...
- [x] D3. Update pgvector queries to filter by `user_id`
...
| User isolation | pgvector `user_id` filter + Supabase RLS enforce data boundaries |
...
| pgvector embeddings | `user_id` column + RLS |
...
- [x] pgvector searches are scoped to authenticated user
...
**Problem:** Logs are currently scattered across 5+ dashboards (Cloudflare, Modal, Supabase, pgvector, ElevenLabs). When something fails, it's hard to trace what happened.
...
CREATE TABLE public.operation_costs (
  ...
  service TEXT NOT NULL,            -- modal, elevenlabs, claude, pgvector
...
| `/api/embed-content` | POST | Manually embed episode in pgvector (body: `{episode_id}`) |
...
| Tool | Description |
|------|-------------|
| `search_podcasts` | Semantic search across all transcripts (pgvector) |
...
#### 8.2 pgvector Embeddings Table (Replaces pgvector)
...
export interface Env {
  ...
  OPENAI_API_KEY: string;  // For embeddings generation
}
